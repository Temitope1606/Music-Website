...
// THIS FUNCTION IS FOR DISPLAYING THE PICTURE,TITLE,ARTIST FOR PLAYLIST IN playMusic page
    ...
    public function getTitleArtist($music_picture,$table) {
        ...
        $result = mysqli_query($this->link, $query);
            ...
            echo "<div class='song-navigation'>";
            echo '<form action="" method="post">';
            echo '<input type="hidden" name="previous" value="true">';
            echo '<button type="submit" id="prev-next">' . "&larr;" . '</button>';// For Previous song
            echo '</form>';
            ...
            echo '<button type="submit" id="prev-next">' . "&rarr;" . '</button>';// For Next Song
            echo '</form>';
            echo "</div>";
            /*echo "<audio controls>
    ...
    // TO DISPLAY THE HOME PAGE PLAYLIST
     // By adding onclick="this.closest('form').submit();" to the picture, the picture itself becomes an 
   ...
   //  <input type="hidden"> stores information about the image as in the image path  --> 
    public function homePagePlaylist($table){
        ...
        while ($row = mysqli_fetch_array($result)) {
            $song_title = $row['song_title'];
            $artist = $row['artist'];
            $music_picture = $row['music_picture'];
    
            echo '<div class="item">';
            echo ' <form action="playmusic.php" method="post"> ';
            echo '<input type="hidden" name="music_picture" value="' . $music_picture . '">';
            echo '<img src="' . $music_picture . '" alt="' . $song_title . '" id="eve" onclick="this.closest(\'form\').submit();">';
            echo '</form>';
            echo '<h3>' . $song_title. "\t"  . "By" . "\n" . $artist . '</h3>';
            echo '</div>';
        }
    }
