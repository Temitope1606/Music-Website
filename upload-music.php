...
if (isset($_POST['upload'])) {
    ...
    $audiofileName = $_FILES['audio']['name'];
    $audiofileTmpName = $_FILES['audio']['tmp_name'];
    $audiofileError = $_FILES['audio']['error'];
    ...
    if (in_array($audiofileActualExt, $allowedaudio)) {
        if ($audiofileError === 0) {
            $audiofileDestination = 'songs/' . $audiofileName;
            // print_r("$audiofileDestination");
            move_uploaded_file($audiofileTmpName, $audiofileDestination);
            $_SESSION['audioFile'] = $audiofileDestination;

            echo ("Audio Successfully Uploaded!");
        } else {
            echo "There was an error uploading this audio file!";
        }
    } else {
        echo "You cannot upload audio files of this type!";
    }
    ...
    if (in_array($picturefileActualExt, $allowedPicture)) {
        if ($picturefileError === 0) {
            $picturefileDestination = 'images/' . $picturefileName;
    ...
    $query = "INSERT INTO sample_songlist_table(song_title, artist, music_file, music_picture, genre)";
